...
void OnInit() {
    ...
    worker_add(const_cast<char*>(name.c_str()), name.size(), const_cast<char*>(addr.c_str()), addr.size(), handle);
...
void OnTick() {
    ...
    worker_echo(0, const_cast<char*>(rpay.c_str()), rpay.size());
    worker_echo(1, const_cast<char*>(jsonpay.c_str()), jsonpay.size());
}
...
int main(int argc, char** argv) {
  ...
  printf("%d\n", int_echo(42));
  string_t reply[1024];
  string_t source[] = L"String echo test\n";
